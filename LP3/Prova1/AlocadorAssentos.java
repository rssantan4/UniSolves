...
public class AlocadorAssentos implements Runnable {
    ...
    private void processarRequisicao(RequisicaoReserva requisicao) {
        ...
        try {
            ...
            if (permissaoAdquirida) {
                ...
                } else { //se especifico chama alocaAssento
                    resposta = alocarAssentoEspecifico(requisicao, codigoAssento);
                }
                
            } else {
                resposta = RespostaReserva.falha("TIMEOUT", "Sistema sobrecarregado.");
            }

        } catch (InterruptedException e) {
            Thread.currentThread().interrupt(); //rest flag
            resposta = RespostaReserva.falha("ERRO_SERVIDOR", "Operação interrompida.");
            
        } finally {
            //liberar o Semaphore no bloco finally
            if (permissaoAdquirida) {
    ...
    private RespostaReserva alocarQualquerAssento(RequisicaoReserva requisicao) {
        ...
        try { 
            for (String codigoAssento : mapaAssentos.getTodosCodigosAssentos()) { //iteração
                ...
                if (assentoTrava.tryLock(5, TimeUnit.SECONDS)) {
                    try {
                        //double-check 
                        if (assento.estaOcupado()) {
                            continue; //another thread reserved
                        }
                        //compareAndSet() do AtomicBoolean para garantir atomicidade
                        ...
                        if (flagOcupado.compareAndSet(false, true)) {
                            ...
                            return RespostaReserva.sucesso(codigoAssento, codigoReserva); //deu bom
                        }
                    } finally {
                        assentoTrava.unlock(); //libera lock
                    }
                }
            }
        } catch (InterruptedException e) {
        ...
        return RespostaReserva.falha("VOO_LOTADO", "Voo lotado"); //nenhum disp
    }
